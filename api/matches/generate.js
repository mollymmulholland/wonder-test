...
async function birthMap(){
  try{const rows=await rest('/birth_data?select=user_id,date_of_birth',{admin:true});return Object.fromEntries(rows.map(r=>[r.user_id,r.date_of_birth]));}catch{return{};}
}
...
module.exports=async function handler(req,res){
  ...
  const auth=String(req.headers.authorization||'');const token=auth.startsWith('Bearer ')?auth.slice(7):'';const user=await authUser(token);
  if(!user?.id)return res.status(401).json({error:'Authentication required.'});
  try{
    const births=await birthMap();
    const meRaw=(await rest(`/profiles?user_id=eq.${user.id}&select=*`,{admin:true}))[0]||{};const meProfile=enrich(meRaw,births);
    const meModel=await latestModel(user.id);if(!meModel)return res.status(409).json({error:'Complete the Wonder assessment before generating introductions.'});
    const candidates=(await rest(`/profiles?user_id=neq.${user.id}&select=*`,{admin:true})).map(p=>enrich(p,births));
    ...
    const top=eligible.slice(0,3).map(({candidate_user_id,profile,result})=>({candidate_user_id,first_name:profile.first_name||null,current_city:profile.current_city||null,score:result.score,confidence:result.confidence,components:result.components,rationale:result.rationale,distance_miles:result.hard_details?.distance_miles??null}));
