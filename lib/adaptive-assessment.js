...
function scoreCandidate(item,state){
 ...
 // Questions with no direct trait weights (e.g. shadow hypothesis probes) should not be
 // selected merely to increase psychometric coverage.
 if(!dims.length)return -10;
 for(const d of dims){
...
function nextItem(responses={}){
 ...
 const candidates=ITEMS.filter(x=>!state.answered.has(x.id)&&itemDimensions(x).length).sort((a,b)=>scoreCandidate(b,state)-scoreCandidate(a,state));
 ...
 // Do not stop at 32 solely because coverage exists. Ambiguous archetypes or strong
 // tensions deserve precision questions until 36 or until the ambiguity resolves.
