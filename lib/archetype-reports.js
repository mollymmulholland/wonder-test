...
const VERSION = 'wonder-mirror-library-2026.09.21';
...
function reportFor(name) {
 ...
 return {name,sections:sections(w,null,null,name),lens_status:'provisional',version:VERSION,model_version:MODEL_VERSION,essence:a.essence,elements:a.elements,opening:w[0],gift:a.relational.gift,need:a.relational.need,shadow:a.relational.shadow,
...
function personalizedReport(model,archetypes,mirror){
 ...
 const report=reportFor(primary?.name); if(!report)return {name:'Unassigned',version:VERSION,elements:[],sections:sections(Array(8).fill('There is not enough evidence for a specific interpretation yet.'),model,mirror),lens_status:'unassigned',prompt:'What would you like us to understand first?',chapters:[]};
 const gap=Math.max(0,(primary.score||0)-(secondary?.score||0));
 return {...report,sections:sections(WRITING[primary.name],model,mirror,primary.name),lens_status:gap<.055?'mixed':'provisional',secondary:secondary?.name||null,blend_note:secondary?`Your ${secondary.name} pattern brings ${ARCHETYPES[secondary.name]?.relational.gift}. ${gap<.055?'Your leading patterns are close; the order should remain provisional.':'This adds another way your primary pattern may be expressed.'}`:null,
