(()=>{
  ...
  function saveSession(data,{fallbackEmail='',fallbackPhone=''}={}){
    ...
    current.account={...(current.account||{}),id:u.id||current.account?.id||null,email:u.email||fallbackEmail||current.account?.email||'',phone:u.user_metadata?.phone||fallbackPhone||current.account?.phone||'',emailVerified:!!u.email_confirmed_at,phoneVerified:false,persistence:'supabase',createdAt:u.created_at||current.account?.createdAt||new Date().toISOString()};
    if(data.access_token){const seconds=Number(data.expires_in||3600);current.auth={accessToken:data.access_token,refreshToken:data.refresh_token||current.auth?.refreshToken||null,expiresIn:seconds,savedAt:Date.now(),expiresAt:Date.now()+seconds*1000};}
    save(current);window.dispatchEvent(new CustomEvent('wonder-auth-updated'));
  }

  function cloudTarget(){
    const s=state();let a={};try{a=JSON.parse(localStorage.getItem('wonder_assessment_v2')||'{}')}catch{}
    if(a.sessionId&&!a.complete)return'assessment';
    if(a.complete||s.assessmentV2)return'mirror';
    if(s.essentials&&Object.values(s.essentials).some(Boolean))return'visual';
    if(s.birth&&(s.birth.dob||s.birth.pob||s.birth.tob))return'essentials';
    return'birth';
  }
  ...
  async function hydrateFromCloud(){
    ...
    try{
      const r=await fetch('/api/persist',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`},body:JSON.stringify({action:'hydrate'})});
      const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'Unable to restore your Wonder account.');
      const s=state();
      if(d.birth){s.birth={dob:d.birth.date_of_birth||'',tob:d.birth.time_of_birth||'',pob:d.birth.place_of_birth||'',toa:d.birth.time_accuracy||'Exact'};}
      if(d.profile){s.essentials={firstName:d.profile.first_name||'',currentCity:d.profile.current_city||'',gender:d.profile.gender||'',interested:d.profile.interested_in||'',intent:d.profile.relationship_intention||'',structure:d.profile.relationship_structure||'Monogamy',children:d.profile.children||'',religion:d.profile.religion||'',ageRange:d.profile.age_range||'',distance:d.profile.max_distance||'25 miles',nonnegotiables:d.profile.nonnegotiables||''};}
      const placeMeta={};if(d.birth?.location_data&&Object.keys(d.birth.location_data).length)placeMeta.birthplace=d.birth.location_data;if(d.profile?.location_data&&Object.keys(d.profile.location_data).length)placeMeta.current_city=d.profile.location_data;if(Object.keys(placeMeta).length)localStorage.setItem('wonder_place_meta',JSON.stringify(placeMeta));
      if(d.assessment){s.assessmentV2=d.assessment;s.archetype=d.assessment.mirror?.primary?.name||d.assessment.archetypes?.[0]?.name||s.archetype;}
      save(s);
      if(d.active_assessment?.session){
        localStorage.setItem('wonder_assessment_v2',JSON.stringify({responses:d.active_assessment.responses||{},history:[],sessionId:d.active_assessment.session.id,complete:false,cache:{},changedCounts:{}}));
      }else if(d.assessment){
        localStorage.setItem('wonder_assessment_v2',JSON.stringify({responses:{},history:[],sessionId:d.assessment.assessment_session_id||null,complete:true,cache:{},changedCounts:{},result:d.assessment,snapshotSyncedAt:d.assessment.created_at||new Date().toISOString()}));
      }
  ...
  async function refreshSession(force=false){
    const s=state(),auth=s.auth||{};if(!auth.refreshToken)return false;
    ...
    try{const r=await fetch('/api/signup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'refresh',refresh_token:auth.refreshToken})});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'Session expired.');saveSession(d);return true;}
    catch{const current=state();delete current.auth;save(current);window.dispatchEvent(new CustomEvent('wonder-auth-updated'));return false;}
  }
  ...
  btn.onclick=async()=>{
    ...
    try{
      ...
      if(!response.ok){if(data.code==='account_exists'){setMode('signin');throw new Error('That account already exists. Enter your password to sign in.');}throw new Error(data.error||'Unable to continue.');}
      saveSession(data,{fallbackEmail:e,fallbackPhone:p});password.value='';
      showStatus(mode==='signin'?'Restoring your Wonder…':'Account created.');
      await hydrateFromCloud();
      const target=cloudTarget();showStatus(mode==='signin'?'Welcome back.':'Account created.');setTimeout(()=>show(target),180);
