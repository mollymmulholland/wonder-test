...
function sections(w,model=null,mirror=null){return SECTIONS.map(([id,title,keys,index,question])=>{
 ...
 let body=grounded?`Your answers place ${model.dimensions[signals[0]]>0?'more':'less'} emphasis on ${LABELS[signals[0]]}${signals[1]?`, alongside ${model.dimensions[signals[1]]>0?'more':'less'} emphasis on ${LABELS[signals[1]]}`:''}. These are your responses to particular questions, not observations of how you behave in every relationship.`:w[index];
