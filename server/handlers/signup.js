...
module.exports = async function handler(req, res) {
  ...
  const email = cleanText(body.email, 321).toLowerCase();
  try {
    ...
    if (action === 'signin') {
      ...
      try {const session = await tokenRequest('password', {email, password: body.password}); setSessionCookies(res, session); clearRecovery(res); return res.status(200).json(publicSession(session));}
        ...
        if (e.status === 400 || e.status === 401) return res.status(400).json({error: 'Email or password is incorrect.'});
        throw e;
      }
    }
    ...
    if (action === 'create' || action === 'resend') {
      ...
      if (settings.mailer_autoconfirm !== false) return res.status(503).json({error: 'Verified account registration is being prepared. You can explore the demo meanwhile.'});
    }
    if (action === 'create' || action === 'resend') {
    ...
    return res.status(503).json({error: ['create','recover','resend'].includes(action) ? 'We could not send the account email. Please try again later; the demo remains available.' : 'The account service could not complete this request. Please try again.'});
  }
};
