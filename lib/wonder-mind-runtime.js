...
function buildSystem({regions,knowledge,sources,context,dyadContext,runType,purposes,ethics}){
...
async function runMind({userId,runType='chat',message='',history=[],payload={}}){
  ...
  try{
    ...
    const generated=await generate({system:buildSystem({regions,knowledge,sources,context,dyadContext,runType,purposes,ethics:ethicsPreflight}),messages,responseSchema:RESPONSE_JSON_SCHEMA,reasoningMode:mode});
    ...
    const finalOutput=ethicsPost.clear?adjudicated:{...adjudicated,reply:safeAbstention(ethicsPost.violations),claim:'Wonder abstained because the generated conclusion violated a constitutional boundary.',confidence:0,memory_updates:[],predictions:[]};
