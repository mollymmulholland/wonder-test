...
function buildSystem({regions,knowledge,sources,context,dyadContext,evidenceProfile,runType,purposes,ethics}){
...
async function persistPredictions({runId,userId,candidateUserId,predictions=[]}){
  if(!candidateUserId||!predictions.length)return [];
  const body=predictions.map(p=>({run_id:runId,user_id:userId,candidate_user_id:candidateUserId,prediction_type:p.prediction_type,probability:p.probability,horizon:p.horizon,target_definition:{description:p.target_definition},evidence:p.evidence,resolved:false}));
  return rest('/wonder_mind_predictions?select=id,prediction_type,probability',{method:'POST',admin:true,prefer:'return=representation',body});
}

async function persistEvidenceAudit({runId,userId,profile}){
  return rest('/wonder_mind_evidence_audits',{method:'POST',admin:true,prefer:'return=minimal',body:{run_id:runId,user_id:userId,source_families:profile.sourceFamilies,independent_source_count:profile.independentSourceCount,weighted_independence:profile.weightedIndependence,contamination_score:profile.contaminationScore,confidence_ceiling:profile.confidenceCeiling,flags:profile.flags,profile}});
}

async function annotateLearnedMemories(memories,evidenceProfile){
  if(!memories?.length)return;
  const now=new Date().toISOString();
  await Promise.allSettled(memories.map(m=>rest(`/wonder_mind_memory?id=eq.${m.id}`,{method:'PATCH',admin:true,prefer:'return=minimal',body:{source_families:evidenceProfile.sourceFamilies,independent_source_count:evidenceProfile.independentSourceCount,contamination_score:evidenceProfile.contaminationScore,last_external_support_at:evidenceProfile.independentSourceCount>0?now:(m.last_external_support_at||null),confidence:Math.min(Number(m.confidence)||0,evidenceProfile.confidenceCeiling)}})));
}
...
async function runMind({userId,runType='chat',message='',history=[],payload={}}){
  ...
  const runRows=await rest('/wonder_mind_inference_runs?select=*',{method:'POST',admin:true,prefer:'return=representation',body:{user_id:userId,model_version_id:modelVersions[0]?.id||null,run_type:runType,trigger_event_id:event.id,status:'running',started_at:new Date().toISOString(),retrieved_memory_ids:retrievedIds,input_summary:{message:message.slice(0,1200),history_count:history.length,purposes,reasoning_mode:mode,regions:routing.selected,knowledge_count:knowledge.length,source_count:sources.length,candidate_user_id:candidateUserId,dyad_outcomes:dyadContext?.outcomeHistory?.length||0,evidence_independence:{independent_source_count:evidenceProfile.independentSourceCount,contamination_score:evidenceProfile.contaminationScore,confidence_ceiling:evidenceProfile.confidenceCeiling,flags:evidenceProfile.flags}}}});
  const run=runRows[0];
  await persistEvidenceAudit({runId:run.id,userId,profile:evidenceProfile});
  await writeAudit({runId:run.id,userId,action:'run_started',metadata:{runType,purposes,reasoningMode:mode,regionCount:regions.length,candidateUserId,evidenceIndependence:evidenceProfile}});
  try{
    ...
    const generated=await generate({system:buildSystem({regions,knowledge,sources,context,dyadContext,evidenceProfile,runType,purposes,ethics:ethicsPreflight}),messages,responseSchema:RESPONSE_JSON_SCHEMA,reasoningMode:mode});
    const raw=safeJson(generated.text)||normalizeMindOutput({reply:generated.text,epistemic_class:'pattern_hypothesis',confidence:.4,claim:'Model response could not be parsed into the expected cognition schema.',memory_updates:[],predictions:[]});
    const normalized=normalizeMindOutput(raw);
    normalized.memory_updates=(normalized.memory_updates||[]).map(m=>({...m,confidence:constrainConfidence(m.confidence,evidenceProfile),stability:evidenceProfile.independentSourceCount<2&&m.stability==='stable'?'provisional':m.stability}));
    const {output:adjudicated,epistemicAudit}=adjudicate(normalized,{retrievedEvidenceCount:knowledge.length+retrievedIds.length+(dyadContext?.outcomeHistory?.length||0),hasLongitudinalEvidence:hasLongitudinalEvidence(context,dyadContext),thirdPartySubject:ethicsPreflight.thirdPartySubject});
    const independenceConstrained={...adjudicated,confidence:constrainConfidence(adjudicated.confidence,evidenceProfile)};
    ...
    await annotateLearnedMemories(learnedMemories,evidenceProfile);
    const predictions=await persistPredictions({runId:run.id,userId,candidateUserId,predictions:finalOutput.predictions||[]});
    ...
    await rest(`/wonder_mind_inference_runs?id=eq.${run.id}`,{method:'PATCH',admin:true,prefer:'return=minimal',body:{status:'completed',completed_at:completedAt,epistemic_state:{...epistemicAudit,alternatives:finalOutput.alternative_hypotheses,what_would_change_mind:finalOutput.what_would_change_mind,evidence_independence:evidenceProfile},ethics_state:{...ethicsPreflight,...ethicsPost},executive_state:{routing:routing.all,selected:routing.selected.map(r=>r.slug),reasoning_mode:mode,dyad_loaded:Boolean(dyadContext),memory_revisions:learnedMemories.length,predictions_created:predictions.length},output:{reply:finalOutput.reply,judgment_id:judgmentId,model:generated.model,candidate:generated.candidate,reasoning_mode:generated.reasoningMode,usage:generated.usage,finish_reason:generated.finishReason}}});
    ...
    await Promise.allSettled([
      ...
      writeAudit({runId:run.id,userId,action:'run_failed',metadata:{code:error.code||'RUNTIME_ERROR',reasoningMode:mode,candidateUserId,evidenceIndependence:evidenceProfile}})
    ]);
    throw error;
  }
}
