...
async function declinedCandidates(userId){
 try{
  const feedback=await rest(`/match_feedback?user_id=eq.${userId}&reaction=eq.decline&select=match_id`,{admin:true});
  const ids=feedback.map(x=>x.match_id).filter(Boolean);if(!ids.length)return new Set();
  const encoded=ids.map(id=>`"${id}"`).join(',');
  const rows=await rest(`/matches?id=in.(${encoded})&select=matched_user_id`,{admin:true});return new Set(rows.map(r=>r.matched_user_id));
...
module.exports=async function handler(req,res){
 ...
 try{
  ...
  for(const profile of candidates){
   const model=await latestModel(profile.user_id);if(!model)continue;const result=compatibility({profile:meProfile,model:meModel},{profile,model});
  ...
  eligible.sort((a,b)=>b.result.score-a.result.score||b.result.confidence-a.result.confidence);const top=[];
  for(const {candidate_user_id,profile,result} of eligible.slice(0,3)){
   let matchId=null;try{const rows=await rest('/matches?on_conflict=user_id,matched_user_id',{method:'POST',admin:true,prefer:'resolution=merge-duplicates,return=representation',body:{user_id:user.id,matched_user_id:candidate_user_id,score:result.score,rationale:{engine_version:ENGINE_VERSION,confidence:result.confidence,components:result.components,strengths:result.rationale?.strengths||[],tensions:result.rationale?.tensions||[],distance_miles:result.hard_details?.distance_miles??null},status:'suggested'}});matchId=rows?.[0]?.id||null;}catch(e){console.error('persist suggested match',e);}
