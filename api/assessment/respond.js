...
module.exports=async function handler(req,res){
  ...
  try{
    await rest(`/assessment_responses_v2?on_conflict=session_id,item_id`,{
      method:'POST',accessToken:token,prefer:'resolution=merge-duplicates,return=minimal',
      body:{session_id,user_id:user.id,item_id,response,response_time_ms:response_time_ms||null,changed_count:Number(changed_count||0),updated_at:new Date().toISOString()}
    });
