...
async function persistDyad({userId,candidateUserId,finalOutput,eventId,judgmentId,dyadContext}){
  ...
  const evidence=summarizeDyadicEvidence(dyadContext?.outcomeHistory||[]);
  const prior=dyadContext?.dyad?.state||{};
...
async function runMind({userId,runType='chat',message='',history=[],payload={}}){
  ...
  const [{knowledge,sources},context,dyadContext]=await Promise.all([
    loadKnowledge(regions.map(r=>r.id),purposes),
  ...
  try{
    ...
    const {output:adjudicated,epistemicAudit}=adjudicate(raw,{retrievedEvidenceCount:knowledge.length+retrievedIds.length+(dyadContext?.outcomeHistory?.length||0),hasLongitudinalEvidence:hasLongitudinalEvidence(context,dyadContext),thirdPartySubject:ethicsPreflight.thirdPartySubject});
