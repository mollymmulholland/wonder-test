...
const RESPONSE_JSON_SCHEMA={
  name:'wonder_mind_cognition',
  strict:true,
  schema:{
    type:'object',additionalProperties:false,
    required:['reply','epistemic_class','confidence','claim','supporting_evidence','counterevidence','alternative_hypotheses','what_would_change_mind','memory_updates','predictions'],
    properties:{
      reply:{type:'string',minLength:1,maxLength:12000},
      epistemic_class:{type:'string',enum:EPISTEMIC_CLASSES},
      confidence:{type:'number',minimum:0,maximum:1},
      claim:{type:'string',minLength:1,maxLength:4000},
      supporting_evidence:{type:'array',maxItems:12,items:{type:'string',maxLength:1000}},
      counterevidence:{type:'array',maxItems:12,items:{type:'string',maxLength:1000}},
      alternative_hypotheses:{type:'array',maxItems:8,items:{type:'string',maxLength:1000}},
      what_would_change_mind:{type:'array',maxItems:8,items:{type:'string',maxLength:1000}},
      memory_updates:{type:'array',maxItems:5,items:{
        type:'object',additionalProperties:false,
        required:['claim','epistemic_class','confidence','stability','region_slug'],
        properties:{
          claim:{type:'string',minLength:1,maxLength:4000},
          epistemic_class:{type:'string',enum:EPISTEMIC_CLASSES},
          confidence:{type:'number',minimum:0,maximum:1},
      ...
      predictions:{type:'array',maxItems:6,items:{
        type:'object',additionalProperties:false,
        required:['prediction_type','probability','horizon','target_definition','evidence'],
        properties:{
          prediction_type:{type:'string',enum:PREDICTION_TYPES},
          probability:{type:'number',minimum:0.05,maximum:0.95},
          horizon:{type:'string',maxLength:120},
          target_definition:{type:'string',minLength:1,maxLength:500},
...
function normalizeMindOutput(raw={}){
  ...
  return {
    reply:text(raw.reply,12000),
    epistemic_class:cls,
    confidence:clamp(raw.confidence),
    claim:text(raw.claim||raw.reply,4000)||'No substantive claim returned.',
    supporting_evidence:list(raw.supporting_evidence,12),
    counterevidence:list(raw.counterevidence,12),
    alternative_hypotheses:list(raw.alternative_hypotheses,8),
    what_would_change_mind:list(raw.what_would_change_mind,8),
    memory_updates:(Array.isArray(raw.memory_updates)?raw.memory_updates:[]).slice(0,5).map(m=>({claim:text(m?.claim,4000),epistemic_class:EPISTEMIC_CLASSES.includes(m?.epistemic_class)?m.epistemic_class:'pattern_hypothesis',confidence:clamp(m?.confidence),stability:STABILITIES.includes(m?.stability)?m.stability:'provisional',region_slug:REGION_SLUGS.includes(m?.region_slug)?m.region_slug:'self-identity'})).filter(m=>m.claim),
