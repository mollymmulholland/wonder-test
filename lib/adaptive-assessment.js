...
const {assessmentInformationGain}=require('./wonder-mind-curiosity');
...
function scoreCandidate(item,state){
 const dims=itemDimensions(item);if(!dims.length)return -10;
 const answeredInSection=ITEMS.filter(x=>x.section===item.section&&state.answered.has(x.id)).length;
 const recentSectionRepeated=state.lastSections.length===2&&state.lastSections.every(s=>s===item.section);
 const eig=assessmentInformationGain({itemDimensions:dims,evidence:state.model.evidence||{},matchCritical:MATCH_CRITICAL,tensionDims:state.tensionDims,discriminatorDims:state.archetype.dims,answeredInSection,recentSectionRepeated,type:item.type});
 // Tiny deterministic tie-breakers preserve coverage diversity without overpowering information gain.
 return eig+(item.type==='scale'?.004:0)+(Math.max(0,.03-answeredInSection*.004));
}
function nextItem(responses={}){
 const state=selectionState(responses),count=state.answered.size;
 for(const id of CORE_IDS){if(!state.answered.has(id)){const item=ITEMS.find(x=>x.id===id);return{item,phase:'core',count,target_min:28,target_max:36,coverage:state.model.coverage,archetype_gap:state.archetype.gap,tensions:state.tensions.map(t=>t.label),information_gain:item?scoreCandidate(item,state):null}}}

 const candidates=ITEMS.filter(x=>!state.answered.has(x.id)&&itemDimensions(x).length).map(item=>({item,information_gain:scoreCandidate(item,state)})).sort((a,b)=>b.information_gain-a.information_gain);
 ...
 const mature=count>=32&&weakCritical<=3&&severeTensions===0&&sufficientlySeparated;
 const topGain=candidates[0]?.information_gain||0;
 // Stop only when psychometric coverage is acceptable AND the best remaining question has low marginal information value.
 const lowMarginalGain=count>=28&&topGain<.22;
 const shouldStop=(ready&&lowMarginalGain)||mature||count>=36;
 if(shouldStop||!candidates.length)return{complete:true,phase:'complete',count,target_min:28,target_max:36,coverage:state.model.coverage,archetype_gap:state.archetype.gap,tensions:state.tensions.map(t=>t.label),remaining_information_gain:topGain};

 const phase=state.tensions.length||!sufficientlySeparated?'precision':'coverage';
 return{item:candidates[0].item,phase,count,target_min:28,target_max:36,coverage:state.model.coverage,archetype_gap:state.archetype.gap,tensions:state.tensions.map(t=>t.label),information_gain:candidates[0].information_gain};
}
