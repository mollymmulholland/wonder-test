...
const RESPONSE_JSON_SCHEMA={
  ...
  schema:{type:'object',additionalProperties:false,required:['reply','epistemic_class','confidence','claim','supporting_evidence','supporting_evidence_refs','counterevidence','alternative_hypotheses','what_would_change_mind','memory_updates','predictions'],properties:{
    ...
    supporting_evidence:{type:'array',maxItems:12,items:{type:'string',maxLength:1000}},supporting_evidence_refs:{type:'array',maxItems:12,items:{type:'string',minLength:1,maxLength:160}},counterevidence:{type:'array',maxItems:12,items:{type:'string',maxLength:1000}},alternative_hypotheses:{type:'array',maxItems:8,items:{type:'string',maxLength:1000}},what_would_change_mind:{type:'array',maxItems:8,items:{type:'string',maxLength:1000}},
    memory_updates:{type:'array',maxItems:8,items:{type:'object',additionalProperties:false,required:['memory_key','claim','epistemic_class','confidence','stability','region_slug','salience','evidence_refs'],properties:{
      memory_key:{type:'string',minLength:3,maxLength:96,pattern:'^[a-z0-9][a-z0-9-]*$'},claim:{type:'string',minLength:1,maxLength:4000},epistemic_class:{type:'string',enum:EPISTEMIC_CLASSES},confidence:{type:'number',minimum:0,maximum:1},stability:{type:'string',enum:STABILITIES},region_slug:{type:'string',enum:REGION_SLUGS},salience:{type:'number',minimum:0,maximum:1},evidence_refs:{type:'array',maxItems:12,items:{type:'string',minLength:1,maxLength:160}}
    }}},
    predictions:{type:'array',maxItems:6,items:{type:'object',additionalProperties:false,required:['prediction_type','probability','horizon','target_definition','evidence','evidence_refs'],properties:{prediction_type:{type:'string',enum:PREDICTION_TYPES},probability:{type:'number',minimum:0.05,maximum:0.95},horizon:{type:'string',maxLength:120},target_definition:{type:'string',minLength:1,maxLength:500},evidence:{type:'array',maxItems:8,items:{type:'string',maxLength:800}},evidence_refs:{type:'array',maxItems:12,items:{type:'string',minLength:1,maxLength:160}}}}}
  }}
};
...
function normalizeMindOutput(raw={}){
  ...
  return {reply:text(raw.reply,12000),epistemic_class:cls,confidence:clamp(raw.confidence),claim:text(raw.claim||raw.reply,4000)||'No substantive claim returned.',supporting_evidence:list(raw.supporting_evidence,12),supporting_evidence_refs:list(raw.supporting_evidence_refs,12,160),counterevidence:list(raw.counterevidence,12),alternative_hypotheses:list(raw.alternative_hypotheses,8),what_would_change_mind:list(raw.what_would_change_mind,8),
    memory_updates:(Array.isArray(raw.memory_updates)?raw.memory_updates:[]).slice(0,8).map(m=>({memory_key:key(m?.memory_key,`${m?.region_slug||'self'}-${m?.claim||''}`),claim:text(m?.claim,4000),epistemic_class:EPISTEMIC_CLASSES.includes(m?.epistemic_class)?m.epistemic_class:'pattern_hypothesis',confidence:clamp(m?.confidence),stability:STABILITIES.includes(m?.stability)?m.stability:'provisional',region_slug:REGION_SLUGS.includes(m?.region_slug)?m.region_slug:'self-identity',salience:clamp(m?.salience,0,1,.5),evidence_refs:list(m?.evidence_refs,12,160)})).filter(m=>m.claim),
    predictions:(Array.isArray(raw.predictions)?raw.predictions:[]).slice(0,6).map(p=>({prediction_type:PREDICTION_TYPES.includes(p?.prediction_type)?p.prediction_type:null,probability:clamp(p?.probability,.05,.95,.5),horizon:text(p?.horizon,120),target_definition:text(p?.target_definition,500),evidence:list(p?.evidence,8,800),evidence_refs:list(p?.evidence_refs,12,160)})).filter(p=>p.prediction_type&&p.target_definition)};
}
