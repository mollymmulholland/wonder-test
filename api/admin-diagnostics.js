...
async function wonderMindDiagnostics(){
  const [model,regions,sources,knowledge,runs,events,memories,versions,evidenceAudits,driftMetrics,runEvidence]=await Promise.all([
  ...
  const readiness={cognitive_architecture:count(regions)===17,research_corpus:count(sources)>=50&&count(knowledge)>=30,event_memory_layer:true,provenance_ledger:true,epistemic_integrity:epistemicIntegrity,self_hosted_inference:Boolean(model.ok&&model.modelAvailable!==false),constitutional_evaluation:constitutionalEvalPassed,production_ready:Boolean(count(regions)===17&&count(sources)>=50&&count(knowledge)>=30&&model.ok&&model.modelAvailable!==false&&constitutionalEvalPassed&&failedRuns===0&&epistemicIntegrity!=='warning')};
  return{status:readiness.production_ready?'ready':'building',readiness,model,selected_candidate:selected,promotion_gates:PROMOTION_GATES,evaluation:evalSummary,epistemic_integrity:{status:epistemicIntegrity,recent_audits:audits.length,mean_contamination:avgContamination,mean_independent_sources:avgIndependence,invalid_reference_runs:invalidRefRuns,latest_drift:latestDrift},counts:{regions:count(regions),sources:count(sources),knowledge:count(knowledge),active_memories:count(memories),recent_runs:count(runs),recent_failed_runs:failedRuns,recent_failed_events:failedEvents,recent_evidence_rows:count(runEvidence)},model_versions:modelVersions};
}
