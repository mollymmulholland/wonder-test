...
async function runMind({userId,runType='chat',message='',history=[],payload={}}){
  ...
  try{
    ...
    const generated=await generate({system:buildSystem({regions,knowledge,sources,context,runType,purposes,ethics:ethicsPreflight}),messages,responseSchema:RESPONSE_JSON_SCHEMA});
    ...
    await writeAudit({runId:run.id,userId,action:'run_completed',targetId:judgmentRows[0]?.id||run.id,targetType:judgmentRows[0]?.id?'judgment':'inference_run',metadata:{epistemicClass:finalOutput.epistemic_class,confidence:finalOutput.confidence,ethicsClear:ethicsPost.clear,model:generated.model}});
    ...
    await Promise.allSettled([
      ...
      writeAudit({runId:run.id,userId,action:'run_failed',metadata:{code:error.code||'RUNTIME_ERROR'}})
    ]);
    throw error;
  }
}
